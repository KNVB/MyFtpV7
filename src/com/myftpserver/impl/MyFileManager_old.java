...
public class MyFileManager_old extends FileManager 
{
	DbOp dbo=null;
	ResultSet rs=null;
	String strSql=new String();
	public MyFileManager_old(Configuration c) 
	{
		super(c);
		try 
		{
	...
	public long getPathSize(FtpSessionHandler fs, String clientPath)throws AccessDeniedException, PathNotFoundException 
	{
		...
		if (Utility_old.isReadableServerPath(fs,fs.getUser().getServerPathACL(),Paths.get(serverPath)))
		{	
	...
	public void changeDirectory(FtpSessionHandler fs, String inPath)throws AccessDeniedException, PathNotFoundException 
	{
		...
		clientPath=Utility_old.resolveClientPath(logger,fs.getCurrentPath(),inPath);
		...
		{
			if (Utility_old.isReadableServerPath(fs,user.getServerPathACL(),Paths.get(serverPath)))
	...
	public StringBuffer getFullDirList(FtpSessionHandler fs, String inPath)	throws AccessDeniedException, PathNotFoundException,InterruptedException 
	{
		...
		currentPath=Utility_old.resolveClientPath(logger, fs.getCurrentPath(), inPath);
		...
		{	try (DirectoryStream<Path> directoryStream = Files.newDirectoryStream(Paths.get(serverPath))) 
			{
				for (Path path : directoryStream) 
	            {
					...
					if (isVirDirOk && Utility_old.isReadableServerPath(fs,user.getServerPathACL(),path))
		            	result.put((path.getFileName().toString()),Utility_old.formatPathName(path));
	            }
				logger.debug("Client Path ACL size="+user.getClientPathACL().size());
				Utility_old.addVirtualDirectoryList(fs,user.getClientPathACL(),dbo,result);
	...
	public StringBuffer getFileNameList(FtpSessionHandler fs, String inPath)throws AccessDeniedException, PathNotFoundException,InterruptedException 
	{
		...
		currentPath=Utility_old.resolveClientPath(logger, fs.getCurrentPath(), inPath);
		serverPath=dbo.getRealPath(fs, inPath, FileManager.READ_PERMISSION);
		logger.debug("inPath="+inPath+",serverPath="+serverPath+",per="+Utility_old.isReadableServerPath(fs,user.getServerPathACL(),Paths.get(serverPath)));
		if (Utility_old.isReadableServerPath(fs,user.getServerPathACL(),Paths.get(serverPath)))
		{	try (DirectoryStream<Path> directoryStream = Files.newDirectoryStream(Paths.get(serverPath))) 
			{
				for (Path path : directoryStream) 
				...
				logger.debug("Client Path ACL size="+user.getClientPathACL().size());
				if (inPath.equals(""))
					Utility_old.addVirtualDirectoryName(fs.getConfig().getLogger(),fs.getCurrentPath(),user.getClientPathACL(),result);
				else
					Utility_old.addVirtualDirectoryName(fs.getConfig().getLogger(),inPath,user.getClientPathACL(),result);
	...
	{
		User user=fs.getUser();
		String clientPath=Utility_old.resolveClientPath(logger,fs.getCurrentPath(), inPath);
		String serverPath=dbo.getRealPath(fs,clientPath,FileManager.READ_PERMISSION);
		logger.debug("serverPath="+serverPath);
		if (Utility_old.isReadableServerPath(fs,user.getServerPathACL(),Paths.get(serverPath)))
		{	
	...
	public String putFile(FtpSessionHandler fs, String inPath)throws AccessDeniedException, PathNotFoundException,InterruptedException, QuotaExceedException 
	{
		...
		{	
			pathPerm=Utility_old.getServerPathPerm(fs.getConfig().getLogger(),user.getServerPathACL(),Paths.get(serverPath));
			if (pathPerm.indexOf(FileManager.NO_ACCESS)==-1)
			{	
				return serverPath;
			}
